...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    code_review_args.insert("code".to_string(), r#"
...
"#.to_string());
    ...
    match client.get_prompt("code-review", code_review_args).await {
        Ok(result) => {
            ...
            for (i, msg) in result.messages.iter().enumerate() {
                println!("   {}. [{}] {}", 
                    ...
                    msg.role,
                    match &msg.content {
                        pmcp::types::MessageContent::Text { text } => text,
                    }
                );
            }
        }
    ...
    data_args.insert("data".to_string(), 
        "Product,Sales,Revenue\nWidget A,150,4500\nWidget B,200,8000\nWidget C,100,5000".to_string());
    data_args.insert("data_type".to_string(), "CSV".to_string());
    data_args.insert("question".to_string(), "Which product has the highest profit margin?".to_string());
    data_args.insert("output_format".to_string(), "detailed".to_string());
    
    match client.get_prompt("data-analysis", data_args).await {
        Ok(result) => {
            println!("   ✅ Prompt generated!");
    ...
    writing_args.insert("topic".to_string(), "The benefits of Rust for systems programming".to_string());
    ...
    match client.get_prompt("writing-assistant", writing_args).await {
        Ok(result) => {
            println!("   ✅ Prompt generated!");
            if let Some(desc) = &result.description {
                println!("   Description: {}", desc);
    ...
    match client.get_prompt("code-review", incomplete_args).await {
        Ok(_) => {
