...
use pmcp::{
    ...
    StdioTransport, ToolHandler, Error, ErrorCode,
...
impl ToolHandler for ErrorDemoTool {
    async fn handle(&self, arguments: Value) -> pmcp::Result<CallToolResult> {
        let scenario = arguments.get("scenario")
            .and_then(|v| v.as_str())
            .unwrap_or("success");
        ...
        match scenario {
            ...
            "timeout" => {
                ...
                Ok(CallToolResult {
                    content: vec![json!({"status": "should_timeout"})],
                    is_error: false,
                })
            }
            ...
            "validation" => {
                ...
                if input.len() < 5 {
                    Err(Error::validation("Input must be at least 5 characters long"))
                } else if !input.chars().all(|c| c.is_alphanumeric()) {
                    Err(Error::validation("Input must contain only alphanumeric characters"))
                } else {
                    Ok(CallToolResult {
                        content: vec![json!({
                            "status": "validated",
            ...
            Err(e) => {
                ...
                match &e {
                    ...
                    _ => {
                        ...
                        if msg.contains("temporary") || msg.contains("retry") {
                            // Retryable
                        } else {
                            return Err(e);
                        }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (scenario, description) in error_scenarios {
        ...
        match client.call_tool("error_demo", json!({"scenario": scenario})).await {
            ...
            Err(e) => {
                ...
                if let Error::JsonRpcError { data, .. } = &e {
    ...
    for (input, description) in test_inputs {
        ...
        match client.call_tool("error_demo", json!({
            "scenario": "validation",
            "input": input
        })).await {
            Ok(result) => {
                println!("✅ {}", result.content[0].get("status").unwrap());
            }
            Err(e) => {
                println!("❌ {}", e);
            }
    ...
    let result = retry_with_backoff(
        || {
            ...
            Box::pin(async move {
                client.call_tool("error_demo", json!({"scenario": "transient"})).await
            })
        },
        3,
    ...
    match tokio::time::timeout(
        ...
        client.call_tool("error_demo", json!({"scenario": "timeout"}))
    ).await {
    ...
    let primary_result = client.call_tool("error_demo", json!({
    ...
    match primary_result {
        ...
        Err(e) => {
            ...
            match client.call_tool("error_demo", json!({"scenario": "success"})).await {
