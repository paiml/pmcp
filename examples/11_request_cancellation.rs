...
use pmcp::{
    ...
    types::{CallToolResult, RequestId, CancelledNotification}
};
...
impl ToolHandler for CancellableTool {
    async fn handle_with_cancellation(
        ...
        let duration_secs = arguments.get("duration")
            .and_then(|v| v.as_u64())
            .unwrap_or(5);
...
impl ToolHandler for ResourceIntensiveTool {
    async fn handle_with_cancellation(
        ...
        let resource_count = arguments.get("resources")
            .and_then(|v| v.as_u64())
        ...
        for i in 0..resource_count {
            ...
            if pmcp::is_request_cancelled(&request_id).await {
                println!("🛑 Allocation cancelled, cleaning up {} resources", allocated_resources.len());
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client.on_cancelled(move |notification: CancelledNotification| {
        println!("📢 Server confirmed cancellation of request: {:?}", notification.request_id);
    ...
    for handle in handles {
        match handle.await {
            Ok((id, Ok(Ok(result)))) => {
                let status = result.content[0].get("status")
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown");
